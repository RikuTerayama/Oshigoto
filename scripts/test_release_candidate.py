...
def main() -> int:
    ...
    for path in PUBLIC_HTML_PATHS:
        ...
        if path == "/":
            require(not soup.select(".hero-tool-grid > .hero-tool-card"), "/: duplicate hero tool catalog detected")
            require(len(soup.select(".landing-tools-zone")) == 1, "/: expected one canonical tool catalog")
